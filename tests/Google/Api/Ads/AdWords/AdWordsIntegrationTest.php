...
class AdWordsIntegrationTest extends PHPUnit_Framework_TestCase {
  ...
  /**
   * Set up the test function data
   */
  public function setUp() {
    date_default_timezone_set('America/New_York');
    $this->assetHelper = new AssetHelper(dirname(__FILE__));
  }

  /**
   * Tear down the test function data
   */
  ...
  public function testIntegrationCheckUserAgent() {
    ...
    $xmlResponse = $this->assetHelper->getAsset(
  ...
  /**
   * Tests that the user access_token is properly set in the URL for this
   * client library.
   */
  ...
  /**
   * Tests that access_token is properly converted into a URL parameter for
   * this client library when it's valid.
   */
  public function testIntegrationOAuth2Handler_ValidAccessToken() {
    ...
    $oAuth2Info = $user->GetOAuth2Info();
    $user->SetOAuth2Info(array_merge($oAuth2Info, $credentialsOverride));
